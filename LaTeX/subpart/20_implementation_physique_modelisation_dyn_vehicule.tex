...
\begin{itemize}[label=$\star$]
\item \texttt{F\_x\_front} : C'est la force de traction générée par les pneus avant en fonction du glissement longitudinal. Calculée par : $$2 \times C_x \times \texttt{slip}$$
...
Ces forces sont nécessaires pour pouvoir modéliser le comportement du véhicule lorsqu'on va changer le véhicule de direction. Nous allons maintenant les utiliser pour calculer les accélérations du véhicule (longitudinale, latéral et lacet), plus seulement grâce aux Lois de Newton comme à la première étape, mais avec les données que nous avons maintenant.
...
%\center{Formules disponible sur : \cite{VDS_MathWorks}
...
Et, nous pouvons donc maintenant visualiser, grâce à Gnuplot, la trajectoire de notre véhicule, en lui donnant un certain angle de braque, une certaine vitesse, orientation, etc. Voici un exemple de simulation sur 1000 itérations, avec un angle de braquage delta fixe, que l'on inverse au bout de 500 itérations.
Voici