\section{L'implémentation de la physique}\label{sec:l'implementation-de-la-physique-/-modelisation-d'un-systeme-de-dynamique-de-vehicule}
\subsection{Introduction}\label{subsec:introduction}
Pour représenter la relation entre un véhicule et la route d'une manière se rapprochant de la réalité, nous avons donc dû implémenter différents phénomènes physiques.
...
Comme base de notre projet, on émet avant tout, l'hypothèse que le véhicle du modèle est une voiture traction.
...
Avec :
\begin{itemize}
    \item $\vec{F}$ la force appliquée sur le corps,
    \item $m$ la masse du corps,
    \item $\vec{a}$ l'accélération résultante.
\end{itemize}
...
Ce processus est décrit par des \glspl{equationDifferentielle}\textsuperscript{*}.
...
Cette méthode repose sur le fait qu'une \gls{derivee} $x$ notée $\dot{x}$ reste constante sur un petit intervalle de temps $dt$.
...
Nous calculons cette valeur \textit{via} un appel à la fonction \texttt{computeYawAcceleration(torque);} où cette relation est implémentée.
...
Ensuite, pour valider le bon fonctionnement des principes physiques que nous avons décrits et implémentés, nous avons réalisé une simulation.
Celle-ci s'étend sur une durée de 10 secondes (avec un pas de 0,1 s), et avons utilisé \textbf{\glspl{gnuplot}\textsuperscript{*}} pour visualiser les résultats obtenus et observer si le résultat obtenu est celui attendu.
...
void simulation_etape1() {
    ...
    // Affichage des données stockées dans les vecteurs avec Gnuplot
    Gnuplot gp;
...
L'\textbf{\gls{angle_glissement}} est l'angle formé entre la direction réelle d'un pneu et l'orientation de la roue.
...
Avec toutes ces nouvelles données, nous pouvons ainsi modéliser la dynamique du \texttt{slip} par une équation du premier ordre :
...
\subsection{Limites des pneumatiques}\label{subsec:limites-des-pneumatiques}
...
$$F_{z\_rear} = m \times g \times \frac{a}{a+b}$$
...
\begin{align}
    \texttt{quotient\_front} &= \frac{\texttt{F\_y\_front}}{\texttt{F\_y\_front\_max}} \\
    \texttt{quotient\_rear} &= \frac{\texttt{F\_y\_rear}}{\texttt{F\_y\_rear\_max}}
\end{align}
...
\begin{itemize}
    \item \texttt{F\_y\_front} : $$\texttt{F\_y\_max\_front} \times \tanh(\texttt{quotient\_front})$$
    \item \texttt{F\_y\_rear} : $$\texttt{F\_y\_max\_rear} \times \tanh(\texttt{quotient\_rear})$$
\end{itemize}

Ici, nous utilisons la fonction $\tanh$, car elle se comporte de manière linéaire pour des faibles valeurs de quotient, et elle tend vers une valeur asymptomatique $(1)$ lorsque le quotient augmente, ce qui limite la force latérale.
...
Après avoir conçu une physique qui répondait aux besoins de notre simulateur, nous avons décidé d'utiliser \glspl{Gnuplot} pour pouvoir afficher les données produites par notre modèle, et ainsi pouvoir visualiser les réactions des différents paramètres utilisés dans notre simulation.
...
De plus, pour lancer la génération de tracés des différentes données de notre implémentation, il faut lancer l'exécutable \texttt{build/DrivingSim\_Plots} \texbf{avec les permissions administrateur}, ce qui générera les graphiques du scénario qui est décrit dans le fichier \texttt{srcPlots/vehicle.cpp}.
