...
\newglossaryentry{std_deque}{
    name={std::deque},
    description={Structure de données de la STL \gls{cpp} utilisée pour gérer une file de deltas de temps}
}

\newglossaryentry{std_vector}{
    name={std::vector},
    description={Conteneur dynamique de la STL \gls{cpp} servant à stocker des données, par exemple, les points de la trajectoire prédictive}
}

\newglossaryentry{std_nth_element}{
    name={std::nth\_element},
    description={Algorithme de la STL \gls{cpp} permettant d’obtenir efficacement l’élément médian dans un conteneur non trié}
}

\newglossaryentry{std_sort}{
    name={std::sort},
    description={Algorithme de tri de la STL \gls{cpp} utilisé pour ordonner les éléments d’un conteneur}
}

\newglossaryentry{std_unordered_map}{
    name={std::unordered\_map},
    description={Structure de données de la STL \gls{cpp} qui associe des clés à des valeurs, utilisée pour la gestion des ressources (textures, etc.)}
}

\newglossaryentry{std_unique_ptr}{
    name={std::unique\_ptr},
    description={Pointeur intelligent de la STL \gls{cpp} assurant la gestion automatique de la mémoire pour des objets, par exemple, la police de caractères}
}
...
%    \glsaddall
    ...
    \begin{thebibliography}{9}
        ...
        Contributeurs de sfml-dev.org.\\
        ``\texttt{sf::VertexArray} - \gls{sfml} Documentation''.\\
