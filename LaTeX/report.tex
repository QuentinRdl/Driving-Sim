...
void computeTranslation(const double Fx, const double Fy, double& ax, double& ay) {
    ...
    où $v_x$ et $v_y$ représentent les composantes de la vitesse et $r$ l'orientation du véhicule.
    ...
    Cette méthode permet d'approximer l'évolution d'une variable, sur un temps fixe, en supposant que sa dérivée soit constante sur un petit intervalle de temps $dt$. Alors, la mise à jour d'une variable $x$ avec sa dérivée $\dot{x}$ suit la relation :
    $$x(t+dt) \approx x(t) + \dot{x}(t) \times dt$$
    Et donc, dans notre cas, nous avons
